...
router.post('/settingSave', function(req, res){
    ...
    db.serialize(()=>{                        
        ...
        if(data.torrentDownloadDir != '') intoDirPath.run('다운로드' , data.torrentDownloadDir);
        if(data.tvProgramDir != '') intoDirPath.run('TV 폴더' , data.tvProgramDir);
        if(data.movieDir != '') intoDirPath.run('영화 폴더' , data.movieDir);
        intoDirPath.run('WATCH 폴더' , data.torrentWatchDir);
